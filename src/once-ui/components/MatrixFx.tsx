  ...
  (
    ...
    useEffect(() => {
      ...
      const parsedColors = colors.map((color) => {
        ...
        const computedColor = getComputedStyle(container).getPropertyValue(
          `--${color}`,
        );
      ...
      const animate = (currentTime: number = 0) => {
        ...
        if (bulgeEnabled) {
          ...
          if (adjustedTime >= 0) {
            ...
            if (bulgeRepeat) {
              ...
              waveProgress =
                cycleTime < bulgeDuration ? cycleTime / bulgeDuration : 0;
        ...
        if (trigger === "instant") {
          ...
          if (isCurrentlyStatic && isStaticRef.current && cachedImageData) {
              ...
              if (bulgeEnabled && showBulge) {
                if (bulgeType === "ripple") {
                  ...
                  const displacementAmount =
                    waveFactor * bulgeIntensity * bulgeFadeOut;
                  ...
                  const distanceToWaveFront =
                    dotDiagonalPos - wavePosOnDiagonal;
            ...
            if (isCurrentlyStatic) {
              cachedImageData = ctx.getImageData(
                0,
                0,
                canvas.width,
                canvas.height,
        ...
        if (trigger === "mount") {
              ...
              if (opacity > 0) {
                ...
                if (bulgeEnabled) {
                  if (bulgeType === "ripple") {
                    ...
                    const wavePosOnDiagonal =
                      waveProgress * diagonalLength * 1.2;
                    const normalizedX = dot.x / canvasWidth;
                    const normalizedY = 1 - dot.y / canvasHeight;
                    const dotDiagonalPos =
                      ((normalizedX + normalizedY) / 2) * diagonalLength;
                    ...
                    const distanceNorm =
                      Math.abs(distanceToWaveFront) / waveWidth;
                    const waveFactor = Math.exp(
                      -distanceNorm * distanceNorm * 3,
                    );
                    const perpendicularOffset = normalizedY - normalizedX;
          ...
          const isCurrentlyStatic =
            mountAnimationCompleteRef.current &&
            !flicker &&
            (!bulgeEnabled || !showBulge);
          if (!isCurrentlyStatic) {
            animationRef.current = requestAnimationFrame(animate);
          } else {
            // Clear the ref so the animation can be restarted
        ...
        if (
          ...
          if (isHoveredRef.current) {
            ...
            dots.forEach((dot) => {
              const normalizedDistance = dot.distanceFromOrigin / maxDistance;
              const introOffset =
                normalizedDistance * 0.8 + dot.randomOffset * 0.5; // Much lower threshold
              ...
              if (reverseProgress > 0.01) {
                ...
                dots.forEach((dot) => {
                  const normalizedDistance =
                    dot.distanceFromOrigin / maxDistance;
                  const introOffset =
                    normalizedDistance * 0.8 + dot.randomOffset * 0.5;
        ...
        const hasActiveAnimation =
          isHoveredRef.current || hideStartProgressRef.current > 0;
    ...
    useEffect(() => {
      ...
      if (active) {
        ...
        if (hideStartProgressRef.current > 0) {
          ...
          const effectiveElapsed = Math.pow(
            currentProgress / (speed * 3),
            1 / 3,
          );
